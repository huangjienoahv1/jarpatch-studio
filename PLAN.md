...
技术路线固定为：`Electron 前端 + Java 后端 + 本地 HTTP API + WebSocket 进度日志 + SQLite 本地历史库`。当前仓库名为 `jarpatch-studio`，许可证使用 `Apache-2.0`。
...
- 第四步补产品化能力：
  ...
  - 只有一次性交付、迁移和归档文档使用日期开头；README、发布清单、构建脚本等持续维护入口使用稳定文件名。
...
## Assumptions
- 产品名固定为 `JarPatch Studio`，代码、脚本和文档使用仓库相对路径，不绑定旧开发目录。
- 第一版完全开源，许可证为 `Apache-2.0`。
- Windows 已完成发布包和真实样本验收；macOS/Linux 构建入口已交付，产物必须在对应原生系统验证。

## 2026-08-10 路线图完成状态

以下能力已经按安全、源码保真、导出闭环、运行稳定、产品交付和跨平台治理顺序完成。

### 第一组：运行态补齐

- 前端已接入带令牌握手的 `/ws/tasks/{taskId}`，实时日志与持久化日志并存。
- 导入、分析、编译和导出已支持取消、条件状态更新和重启恢复。
- 二进制文件、签名文件、超大文件已提供只读提示和风险说明。

### 第二组：配置与排查

- JDK 路径配置页面已经完成，保存时会校验 `bin/javac` 可用性并写入 SQLite。
- 项目级设置已覆盖默认导出目录、嵌套 Jar、文件大小限制和界面偏好。
- 导出前差异视图已展示源码、资源、哈希和 class 清单。
- 导出后已自动校验 Manifest、包布局、资源、class、嵌套 Jar 和签名策略。
- 错误排查向导已覆盖 JDK、CFR、编译、签名、路径和端口问题。

### 第三组：清理与交付

- “只清理本地工作区”已采用预览和一次性确认，并与删除历史分开。
- macOS/Linux 已提供稳定启动和原生打包脚本。
- Windows 免安装包已生成，发布目录包含 SHA-256 清单。
- 真实样本已覆盖普通 JAR、Spring Boot JAR、WAR、签名包、多版本 JAR 和混淆包。
