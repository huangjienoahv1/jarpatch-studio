...
每个目录必须包含 `release-manifest.json`，其中登记版本、平台、架构、JDK、Node.js、npm、文件大小和 SHA-256。
...
- Windows 桌面包：没有组织 Authenticode 证书时必须在发布说明标为“未签名”，不得宣称已获得可信发布者签名。
- macOS 桌面包：公开分发前必须使用 Developer ID 签名并完成 notarization；未配置证书时仅作为内部未签名构建。
- Linux 桌面包：如发布渠道要求 GPG 或仓库签名，必须在渠道侧完成并记录密钥标识。
